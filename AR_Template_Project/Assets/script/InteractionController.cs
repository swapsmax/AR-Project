...
public class InteractionController : ARBaseGestureInteractable
{
    ...
    [SerializeField]
        ...
        void CrosshairCalculation()
        {
            ...
            if (GestureTransformationUtility.Raycast(origin, s_Hits, xrOrigin))
            {
                pose = s_Hits[0].pose;
        ...
        protected override void OnEndManipulation(TapGesture gesture)
        {
            ...
            if (IsPointerOverUI(gesture))
                return;
            if (GestureTransformationUtility.Raycast(gesture.startPosition, s_Hits, xrOrigin))
            {
                var hit = s_Hits[0];
                if (Vector3.Dot(Camera.main.transform.position - hit.pose.position,
                                 hit.pose.rotation * Vector3.up) < 0)
                             return;
                GameObject placementObject = Instantiate(Datahandler.Instance.GetLight(), pose.position, pose.rotation);
